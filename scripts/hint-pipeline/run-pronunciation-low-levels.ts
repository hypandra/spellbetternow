/**
 * Pronunciation pipeline for level 1-4 words (missing IPA/phonetic).
 * Same 3-step flow as run-pronunciation.ts but targeting lower levels.
 *
 * 1. Fetch words from DB (level <= 4, ipa IS NULL)
 * 2. Fetch IPA from Wiktionary
 * 3. LLM fills IPA gaps + generates phonetic respelling for all
 * 4. Output SQL + review JSON
 *
 * Usage: bun scripts/hint-pipeline/run-pronunciation-low-levels.ts [--dry-run]
 */
...
async function main() {
  ...
  console.log('=== SBN Pronunciation Pipeline (Levels 1-4) ===\n');
  console.log(`Options: { dryRun: ${dryRun} }\n`);

  // Step 1: Fetch level 1-4 words missing IPA
  console.log('--- Step 1: Fetch level 1-4 words ---');
  const allWords = await fetchWords({ force: true });
  const words = allWords.filter((w) => w.level <= 4);
  console.log(`  Filtered to ${words.length} words (level 1-4)\n`);
  ...
  const sqlPath = `${OUTPUT_DIR}pronunciation-low-levels-${today()}.sql`;
  ...
  const jsonPath = `${OUTPUT_DIR}pronunciation-low-levels-${today()}.json`;
  ...
  console.log(`\nNext: review JSON, then run ipa-dict upgrade, combine into migration.`);
}
